  ...
  if (gridSliderElement && gridDotsContainer) {
    ...
    const updateGridDots = () => {
      ...
      for (let i = 0; i < slideCount; i++) {
        ...
        dot.classList.add('w-5', 'h-5', 'rounded-full', 'border', 'flex', 'items-center', 'justify-center', 'transition-all', 'duration-300');
        dot.classList.add(i === currentSlide ? 'border-white' : 'border-transparent');
        dot.innerHTML = '<span class="w-2 h-2 bg-white rounded-full block"></span>';
        dot.addEventListener('click', () => gridKeenSliderInstance?.moveToIdx(i));
    ...
    new KeenSliderInstance(gridSliderElement, { loop: false, slides: { perView: 1, spacing: 0 } });
